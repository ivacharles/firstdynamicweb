...
public class TicTacToeTest2 {
    ...
    @DisplayName("X: Find winner on line 2 in position (0, 1, 2)")
    @Test
    void T1() {
        final char [] moves = {'X','X','?','X','X','X','?','?','?'};
    ...
    @DisplayName("O: Find winner on line 2 in position (0, 1, 2)")
    @Test
    void T2() {
        final char [] moves = {'O','O','?','O','O','O','?','?','?'};
        GameStatus gameStatus = TicTacToeBusinessObj.isWinner(moves);
        char expected = 'O';
        char actual = gameStatus.getWhoWon();
        Assertions.assertEquals(expected, actual);
    ...
    @DisplayName("No winner was found on line 2 in position (0, 1, 2)")
    @Test
    void T3() {
